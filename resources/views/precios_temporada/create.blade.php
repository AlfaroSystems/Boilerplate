...
<div class="max-w-xl mx-auto">
    ...
    <div class="bg-white dark:bg-[#161615] rounded-xl shadow p-6 border dark:border-[#3E3E3A]">
        <form action="{{ route('precios-temporada.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="habitacion_id" required class="w-full p-2 border rounded dark:bg-[#1C1C1B] dark:border-[#3E3E3A] dark:text-white">
                    ...
                    @foreach($habitaciones as $habitacion)
                        <option value="{{ $habitacion->id }}">Habitación #{{ $habitacion->numero_habitacion }} ({{ $habitacion->tipo }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="descripcion" placeholder="Ej: Navidad, Verano, Semana Santa" required class="w-full p-2 border rounded dark:bg-[#1C1C1B] dark:border-[#3E3E3A] dark:text-white">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block font-semibold text-gray-700 dark:text-gray-300">Fecha Inicio</label>
                    <input type="date" name="fecha_inicio" required class="w-full p-2 border rounded dark:bg-[#1C1C1B] dark:border-[#3E3E3A] dark:text-white">
                </div>
                <div>
                    <label class="block font-semibold text-gray-700 dark:text-gray-300">Fecha Fin</label>
                    <input type="date" name="fecha_fin" required class="w-full p-2 border rounded dark:bg-[#1C1C1B] dark:border-[#3E3E3A] dark:text-white">
                </div>
            </div>

            <div>
                <label class="block font-semibold text-gray-700 dark:text-gray-300">Precio Especial (Noche)</label>
                <input type="number" step="0.01" name="precio" required class="w-full p-2 border rounded dark:bg-[#1C1C1B] dark:border-[#3E3E3A] dark:text-white">
            </div>
